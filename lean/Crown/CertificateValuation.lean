...
theorem lex_first (s : Fin 256 → Bool) (d : Fin 8 ≃ Fin 8) (i : Fin 256)
    (h : ∀ j > i, permuteMask d j = j) : lexBefore s d i = true := by
  apply (lexBefore_iff s d i).mpr
  intro j hj
  rw [h j hj]

theorem lex_skipped (s : Fin 256 → Bool) (d : Fin 8 ≃ Fin 8)
    (i previous : Fin 256) (hip : i < previous)
...
theorem lex_comparison (s : Fin 256 → Bool) (d : Fin 8 ≃ Fin 8) (i : Fin 256)
...
theorem count_zero (s : Fin 256 → Bool) (i : Nat) :
    value s (.count i 0) = true := counter_zero (selectorNat s) i

theorem count_above (s : Fin 256 → Bool) (i j : Nat) (h : i < j) :
    value s (.count i j) = false := counter_above (selectorNat s) i j h

theorem count_step (s : Fin 256 → Bool) (i : Fin 256) (j : Nat) :
    value s (.count (i.val + 1) (j + 1)) =
      (value s (.count i.val (j+1)) ||
        (value s (.select i) && value s (.count i.val j))) := by
  simpa [value, selectorNat, i.isLt] using counter_step (selectorNat s) i.val j

theorem count_final (s : Fin 256 → Bool) :
    value s (.count 256 9) = false ↔ Fintype.card {m // s m = true} ≤ 8 := by
  exact (counter_eight_iff (selectorNat s)).trans (by rw [prefixCount_selector])

theorem core_true_iff (s : Fin 256 → Bool) (masks : List (Fin 256)) :
    value s (.core masks) = true ↔ ∀ m ∈ masks, s m = true := by
  simp [value, List.all_eq_true]

theorem core_implies_select (s : Fin 256 → Bool) (masks : List (Fin 256))
    (m : Fin 256) (hm : m ∈ masks) :
...
theorem counter_gate (s : Fin 256 → Bool) (i : Fin 256) (j : Nat)
...
theorem equality_gate (s : Fin 256 → Bool) (d : Fin 8 ≃ Fin 8) (i : Fin 256)
...
theorem comparison_gate (s : Fin 256 → Bool) (d : Fin 8 ≃ Fin 8) (i : Fin 256)
    (hmax : toColex (relabelSelector s d) ≤ toColex s)
...
theorem core_gate (s : Fin 256 → Bool) (masks : List (Fin 256))
    (m : Fin 256) (hm : m ∈ masks) (a : Nat → Bool) (z x : Sat.Literal)
